...
static const char to_base64[] =
             ...
             "0123456789+/";
//===============================================
std::string GBase64::encode(const std::vector<BYTE>& buf) {
    if (buf.empty())
        return ""; // Avoid dereferencing buf if it's empty
    return encode(&buf[0], (unsigned int)buf.size());
}
//===============================================
std::string GBase64::encode(const BYTE* buf, unsigned int bufLen) {
...
//===============================================
std::vector<BYTE> GBase64::decode(std::string encoded_string) {
